...
class RequestStateCodec {
    ...
    String encode(JsonObject state, String principal, String requestId) {
    ...
    DecodedState decode(String token) throws RequestStateException {
        ...
        try {
            ...
            throw new RequestStateException("Invalid requestState encoding");
        }

        int dotIndex = combined.lastIndexOf('.');
        if (dotIndex < 0) {
            throw new RequestStateException("Invalid requestState format");
        }
        ...
        if (!MessageDigest.isEqual(
            ...
            throw new RequestStateException("requestState signature verification failed");
        }
        ...
        try (var reader = Json.createReader(new StringReader(json))) {
            ...
            throw new RequestStateException("Invalid requestState payload");
        }

        long expiresAt = payload.getJsonNumber("expiresAt").longValue();
        if (System.currentTimeMillis() > expiresAt) {
            throw new RequestStateException("requestState has expired");
        }
    ...
    record DecodedState(JsonObject state, String principal, String requestId) {}
